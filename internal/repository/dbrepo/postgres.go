...
// SearchAvailabilityByRoom returns true if availability exists, and false if no availability exists for roomID
...
// func GetUserID returns user by id
func (m *postgresDBRepo) GetUserByID(id int) (models.User, error) {
	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
	defer cancel()
	query := `select id, firest_name, last_name, password, access_level, created_at, updated_at
...
func (m *postgresDBRepo) GetReservationsById(id int) (models.Reservation, error) {
	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
	defer cancel()
...
// Updates Processed for reservation by ID
